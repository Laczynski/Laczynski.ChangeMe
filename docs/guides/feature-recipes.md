...
1. Create or update the endpoint in `src/ChangeMe.Backend.Web/Endpoints/<Feature>/` (use `<Feature>/<ChildResource>/` for nested routes â€” see **Feature and sub-slice layout** in [backend-guidelines.md](backend-guidelines.md)).
...
3. Template: `<dg-prime-data-grid [grid]="grid">` with `qgColumn` templates per column; `qgEmpty` for empty state.
