...
| Protected stable `vX.Y.Z` tag | Runs all verification jobs | Builds and publishes the tagged package | Generates manual jobs for the package and configuration from the tagged commit |
...
| `test` | `e2e:verify` | Starts PostgreSQL and MailHog services, installs the pinned Node.js and Playwright Chromium versions, and runs the browser journeys against the real frontend and backend. A Playwright report is retained on failure. |
...
| `build` | `release:package` | For a stable tag, combines the verified frontend and published backend into one deterministic archive and creates its manifest and SHA-256 file. It waits for frontend, backend, and E2E verification. |
