...
This workflow is for the generated application's GitLab package. If `template-pack/ChangeMe.Templates.csproj` exists, first confirm that the user wants an application release rather than the template NuGet release — use `/template-publish` and [maintainer/publishing.md](../../maintainer/publishing.md) instead.
